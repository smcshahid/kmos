...
- OPEN (top pre-GA blocker) — **read-model recovery on boot**: KMOS services keep authoritative in-memory repositories that are logged but NOT rebuilt from the log on start, so after a restart the event log + search projection recover but object DETAIL (`GET /knowledge/:id`) does not (data is safe in the durable log; the running system is degraded across restarts). This mandates `replicas: 1`. Making repositories replay-rebuilt projections is the read-model-persistence refactor.
