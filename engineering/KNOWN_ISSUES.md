...
- DEFERRED CRIT-1: async EventLog port implemented then reverted to protect the green baseline (full consumer/test propagation needs a typechecked CI env; truncation-prone sandbox). AsyncEventLog contract + Postgres adapter remain as the target.
- DEFERRED HIGH-1: no offline tsc/eslint/registry; ran best-effort strip-types syntax check on all 131 sources (0 failures). Must run npm run verify in CI.
