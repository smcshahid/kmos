...
## D-015 — Podcast Studio and the Content Processing Spine (KCSI-02)
**Status:** ACCEPTED — executed 2026-07-01 (WP1–WP8; Podcast Studio V1 shipped; 320 tests pass/0 fail, fitness clean, conformance COMPLIANT; `@kmos/content-projections` extracted, both flagship apps refactored). Recorded as ADR-0015. Plan: `engineering/KCSI-02-PODCAST-STUDIO-PLAN.md`. Close-out: `engineering/review/21-KCSI-02-CLOSEOUT.md`.
**Final recommendation:** YES — make application development the primary focus for the knowledge/media-light app family (each new app is now mostly composition + pulls its next capability). The one demand-pulled caveat: a media-provider initiative (real ffmpeg/translation/preservation) when a media-heavy app (Media Pipeline) is built on KMOS — not before.

## D-016 — Provider configuration model & operational readiness (ESRI-01)
**Status:** ACCEPTED — executed 2026-07-01 (provider config + openai-compatible adapter shipped; both apps provider-unaware; docs consolidated; 326 tests pass/0 fail, fitness clean, conformance COMPLIANT). Recorded as ADR-0016. Plan: `engineering/ESRI-01-ECOSYSTEM-STABILIZATION-PLAN.md`. Close-out: `engineering/review/22-ESRI-01-CLOSEOUT.md`.
**Context:** Before making applications the primary investment, the ecosystem must be operationally complete and provider independence must be **proven** (switch provider by config, not code). Today knowledge-extraction has one real adapter (Ollama) selected by `if (env)` in each app. The brief + Constitution (Art. IV/V) forbid a provider registry/orchestration framework.
**Decision:** (1) Add a small provider **config model + factory** (`ProviderConfig`, `createKnowledgeExtractionFromConfig`, `extractionConfigFromEnv`) composing the chosen adapter with `withFallback` to the reference — a factory + config, no framework. (2) Add a second real **`openai-compatible`** knowledge-extraction adapter (covers OpenAI/Azure/Groq/DeepSeek/OpenRouter/Mistral/Together via baseUrl+model+key) — proving adding a provider needs no app change. (3) Wire both apps to the config factory (provider-unaware; switching = env only). (4) Document extension points for Speech (already endpoint-agnostic) and Translation (reference only). (5) Consolidate operational docs into one authoritative set (Ecosystem Playbook, Provider Architecture, Packaging Standard, Release Readiness Checklist, Manual Testing Philosophy) cross-referencing existing DEPLOYMENT/OLARES/OPERATIONS/RELEASE docs; **manual testing is the final step, only after all automated gates are green**.
**Consequence (intended):** provider independence is a tested fact; future apps inherit a complete operational standard; no framework debt; human validation focuses on experience, not correctness.

## D-017 — The KMOS Book, verified release engineering & automated packaging (ESRI-02)
**Status:** ACCEPTED — executed 2026-07-02 (The KMOS Book shipped; CI fixed + verified GREEN on a real run; automated release.yml authored; Docker Hub/release state audited). Recorded as ADR-0017. Plan: `engineering/ESRI-02-DOCUMENTATION-AND-RELEASE-READINESS-PLAN.md`. Close-out: `engineering/review/23-ESRI-02-CLOSEOUT.md`.
**Final assessment:** ecosystem operationally complete + legible; releases are a version-tag away (platform); docs self-contained (The KMOS Book). **Recommendation: shift primary investment to product development.** Remainders are demand-pulled (native provider adapters; media-provider initiative; Podcast image publish; per-branch CI backport for #18–21), not platform work.

## D-018 — Platform Phase 1 close, Product Era, and the Future Platform Rule (EPT-01)
**Status:** ACCEPTED-plan (product transition). Recorded as ADR-0018. Plan: `engineering/EPT-01-PRODUCT-TRANSITION-PLAN.md`. Close-out: `engineering/review/24-EPT-01-PRODUCT-ERA.md`.
**Context:** PRs #18–22 merged in order to `main` with **verified green CI** (first fully-integrated pass incl. tsc + real Postgres). Two flagships exist; the second was built mostly by composition. Remaining work is products, not platform.
**Decision:** (1) **Close Platform Phase 1; begin the Product Era.** (2) Produce one ecosystem release (`v1.1.0`: KMOS + Knowledge Studio + Podcast Studio) via automated `release.yml`; GitHub Release = authoritative download; verify not assume. (3) Adopt the **Future Platform Rule** as a permanent principle (Ecosystem Constitution **Article XI**): *no platform enhancement unless demanded by a real application or clear multi-application evidence.* (4) Recommended effort: **~90% applications / ~10% platform** (demand-pulled only). No redesign; no speculative capabilities/frameworks; kernel frozen.
**Consequence (intended):** product-first organization with an evidence-based mandate; platform protected from speculative growth by a permanent rule; one canonical ecosystem release + The KMOS Book + ECOSYSTEM-STATUS make the ecosystem legible and installable without historical context.
**Context:** The ecosystem is functionally/operationally complete; what remained was legibility (a future team must operate KMOS from docs alone) and **verified** release engineering. Evidence-first opened it: `gh run list` showed **CI failing on all four prior initiative PRs (#18–21)** — `npm ci` EUSAGE because `package-lock.json` was never regenerated for the new workspace packages (offline sandbox). Verify, don't assume.
**Decision:** (1) Fix + verify CI first (regenerate the lock; never weaken `npm ci`). (2) Author **The KMOS Book** — one coherent authoritative handbook (7 parts), the primary entry point referencing detail docs. (3) Automate release+packaging on a version tag (tests → image → Docker Hub → Olares `.tgz` → GitHub Release with artifacts/checksums; GitHub Release = authoritative download). (4) Verify tags/releases/runs/Docker Hub via real `gh`/registry calls; record honest outcomes; state what needs a networked/Docker env. (5) Consolidate docs (one source per topic), confirm provider-switching is config-only, keep manual-testing-last. No new functionality; no redesign.
**Consequence (intended):** CI green + verifiable/reproducible releases from a tag; a single handbook makes the ecosystem legible without conversation history; releases produce complete downloadable artifacts; honest evidence replaces assumption.
**Context:** KEAI-01 recommended one application-bearing capability initiative before broad app-building. Podcast Studio is that app. Knowledge Studio (consumer #1) already has pure app-local projections (transcript/chapters/evidence/packaging) and consumes KCSI-01 providers; Podcast Studio needs the identical projections plus new audio/media/acquisition work — making it the concrete second consumer that lets these capabilities be extracted on real evidence.
**Decision:** Build Podcast Studio as a complete product (`products/podcast-studio`) — thin app over `@kmos/sdk` + `@kmos/providers`, orchestrated via the Workflow Service, producing verifiable transcript/chapters/summaries/concepts/evidence/quotes/clips/reels/subtitles/translations/search/package with calm honest UX + persistence/recovery/collections/favorites. **Build first, extract second:** candidate capabilities are hypotheses, extracted only after the app proves them (Article II tests), refactoring BOTH apps and recording rationale/trigger. Offline-honest (paste-transcript path, testable offline), Olares-real (providers via env). No kernel/constitution change; no speculative framework; refine `withFallback` only where proven. WP8 delivers the KCSI-02 Capability Assessment + the final "is the capability layer mature enough to focus on apps?" recommendation.
**Consequence (intended):** A second daily-usable product; the capability spine grows from real second-consumer evidence; KMOS + Knowledge Studio become/stay simpler (shared projections remove duplication); every capability decision recorded; nothing speculative.
