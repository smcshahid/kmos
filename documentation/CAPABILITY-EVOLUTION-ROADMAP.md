...
| **`@kmos/web-acquisition`** — crawl frontier, HTML/readability extraction, URL canonicalization, robots.txt (CSTN-01, ADR-0019) | CrawlStation is the **first and only** web-acquisition app; the logic lives in-product as pure, zero-dep modules (`products/crawl-station/src/{crawler,extract,urls,robots}.ts`). One consumer is not evidence. | A **second independent consumer** needs the same crawl/extract/robots behavior (e.g. a docs-site ingester, a monitoring app, or Media Pipeline acquiring web sources). Two consumers → extract behind a stable acquisition contract. | `capabilities/` (new `@kmos/web-acquisition`) |
...
| 2026-07-02 | CSTN-01 (ADR-0019): **CrawlStation** shipped as flagship #003 with **zero platform changes / zero new capabilities** (Product-Era proof of the Future Platform Rule). Web maps onto existing primitives (Asset/Knowledge/Governance/Search). Added §4 deferred candidate **`@kmos/web-acquisition`** (crawl/extract/URL/robots) — single consumer, trigger = second consumer. Full suite 340 tests / 0 fail; fitness clean, 34 packages. |
...
| 2026-07-01 | ESRI-01 (ADR-0016, review/22): provider independence **proven** — 2nd knowledge-extraction adapter (`openai-compatible` → OpenAI/Azure/Groq/DeepSeek/OpenRouter/Mistral/Together) + config model; both apps provider-unaware (switch = env, not code). No registry/framework. Operational docs consolidated (Playbook, Provider Architecture, Packaging Standard, Release&Docker, Readiness Checklist, Manual Testing Philosophy, doc index). Ecosystem operationally complete for the current app family. |
| 2026-07-02 | ESRI-02 (ADR-0017, review/23): **The KMOS Book** (definitive handbook); CI **verified GREEN** on a real run after fixing the package-lock (was RED on #18–21 — evidence over assumption); automated `release.yml` (tag → image → Olares `.tgz` → GitHub Release + checksums); Docker Hub audited (kmos + knowledge-studio images live/pullable). Final recommendation: **shift primary investment to product development.** |
