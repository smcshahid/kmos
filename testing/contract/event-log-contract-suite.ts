/**
 * Reusable EventLog contract (KMOS-9999 §16 contract tests) — shared suite.
 *
 * `runEventLogContract(label, makeLog)` asserts the semantics every EventLog
 * adapter must honour, independent of storage technology:
 ...
 *
 * `makeLog` returns a FRESH, EMPTY log per call and MAY be async (the real
 * Postgres adapter truncates the table before each case). It is consumed by
 * three callers: `InMemoryEventLog`, `PostgresEventLog` over an in-memory fake
 * `SqlClient`, and `PostgresEventLog` over a REAL Postgres in CI — one contract,
 * three adapters. This module is NOT a `*.test.ts` file, so the runner loads it
 * only via import (no double-registration of tests).
 */
...
let counter = 0;
export function event(type: string): CanonicalEvent {
  counter += 1;
  return createEvent({
    type,
    schemaVersion: '1.0',
...
export function runEventLogContract(
  label: string,
  makeLog: () => MaybePromise<ContractEventLog>,
): void {
