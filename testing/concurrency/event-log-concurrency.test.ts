...
test('optimistic concurrency: a stale expectedVersion is rejected (lost-update prevented)', () => {
  ...
  log.append(stream, e(), { expectedVersion: 0 }); // v1
  log.append(stream, e(), { expectedVersion: 1 }); // v2
  // Two writers both read version 2 then try to write v3; the second loses.
  log.append(stream, e(), { expectedVersion: 2 }); // v3 wins
  assert.throws(() => log.append(stream, e(), { expectedVersion: 2 }), /conflict/i);
  assert.equal(log.currentVersion(stream), 3);
});
...
test('per-stream ordering + monotonic global sequence across interleaved streams', async () => {
  ...
  assert.equal(bus.eventLog.currentVersion(a), 2);
  assert.equal(bus.eventLog.currentVersion(b), 2);
  // Global sequence is a strict increasing total order with no gaps/dupes.
  const seqs = bus.eventLog.read(1).map((s) => s.sequence);
  assert.deepEqual(seqs, [1, 2, 3, 4]);
  // Per-stream versions are 1..n in order.
  assert.deepEqual(bus.eventLog.readStream(a).map((s) => s.streamVersion), [1, 2]);
  assert.deepEqual(bus.eventLog.readStream(b).map((s) => s.streamVersion), [1, 2]);
});
